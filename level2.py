...
class Player(pygame.sprite.Sprite):
    ...
    def draw(self, screen):
        ...
        HP_text = pygame.font.SysFont('Consolas', 32) \
            .render("Здоровье: " + str(self.HP) + '/3',
                    True,
                    pygame.color.Color('White'))
        plane_text = pygame.font.SysFont('Consolas', 32) \
            .render("Сбито самолетов: " + str(counter_planes) + '/8',
                    True,
                    pygame.color.Color('White'))
...
def run():
    ...
    mixer.music.set_volume(0.2)cdc
    mixer.music.play(-1)
    clock = pygame.time.Clock()
    background_image = pygame.image.load('images/level2/background.png')
    ground = pygame.transform.scale(pygame.image.load('images/level2/ground.png'), (384, 114))
    is_dead = False
    ...
    while running:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
                ...
                screen.blit(sound_btn, sound_rect)
        if not is_paused and tank.HP > 0 and counter_planes < 8:
            pygame.mouse.set_visible(False)
            # draw all
